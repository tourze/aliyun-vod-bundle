...
class TranscodeTaskFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $transcodeTask1->setTaskId('transcode_task_001_completed');
        $transcodeTask1->setTemplateId('VOD_TEMPLATE_HD_001');
        $transcodeTask1->setStatus('TranscodeSuccess');
        $transcodeTask1->setProgress(100);
        $transcodeTask1->setCompletedTime(new \DateTimeImmutable('-2 hours'));
        ...
        $transcodeTask2->setTaskId('transcode_task_002_processing');
        $transcodeTask2->setTemplateId('VOD_TEMPLATE_4K_001');
        $transcodeTask2->setStatus('Processing');
        $transcodeTask2->setProgress(65);
        ...
        $transcodeTask3->setTaskId('transcode_task_003_failed');
        $transcodeTask3->setTemplateId('VOD_TEMPLATE_SD_001');
        $transcodeTask3->setStatus('TranscodeFail');
        $transcodeTask3->setProgress(0);
        $transcodeTask3->setErrorCode('InvalidVideo.Format');
        $transcodeTask3->setErrorMessage('视频格式不支持，请检查输入文件格式');
